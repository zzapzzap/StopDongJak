...
class TfPoseEstimator:
    ...
    @staticmethod
    def human_extract_points(npimg, humans, imgcopy=False):
        Save_point_List = []
        total_humans_point = []
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                centers[i] = center
                if i == common.CocoPart.RWrist.value:
                    human_point[common.CocoPart.RWrist.name] = centers[i]; Save_point_List.append(i)
                if i == common.CocoPart.LWrist.value:
                    human_point[common.CocoPart.LWrist.name] = centers[i]; Save_point_List.append(i)
                if i == common.CocoPart.RShoulder.value:
                    human_point[common.CocoPart.RShoulder.name] = centers[i]; Save_point_List.append(i)
                if i == common.CocoPart.LShoulder.value:
                    human_point[common.CocoPart.LShoulder.name] = centers[i]; Save_point_List.append(i)
                total_humans_point.append(human_point)

            for k in Save_point_List:
                cv2.circle(npimg, centers[k], 3, common.CocoColors[k], thickness=3, lineType=8, shift=0)
        return npimg, total_humans_point
